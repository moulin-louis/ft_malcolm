...
void print_arp_packet(const t_packet* packet) {
  ...
  dprintf(1, "\t\tar_sha: %02x:%02x:%02x:%02x:%02x:%02x\n", packet->ar_sha[0], packet->ar_sha[1], packet->ar_sha[2], packet->ar_sha[3], packet->ar_sha[4], packet->ar_sha[5]);
  dprintf(1, "\t\tar_sip: %02x:%02x:%02x:%02x || %s\n", packet->ar_sip[0], packet->ar_sip[1], packet->ar_sip[2], packet->ar_sip[3], inet_ntoa(*(struct in_addr*)packet->ar_sip));
  dprintf(1, "\t\tar_tha: %02x:%02x:%02x:%02x:%02x:%02x\n", packet->ar_tha[0], packet->ar_tha[1], packet->ar_tha[2], packet->ar_tha[3], packet->ar_tha[4], packet->ar_tha[5]);
  dprintf(1, "\t\tar_tip: %02x:%02x:%02x:%02x || %s\n", packet->ar_tip[0], packet->ar_tip[1], packet->ar_tip[2], packet->ar_tip[3], inet_ntoa(*(struct in_addr*)packet->ar_tip));
}
